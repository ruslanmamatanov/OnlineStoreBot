...
async def start_handler(message: Message, state: FSMContext):
    ...
    if user:
        if message.from_user.id in admins:
            ...
            await message.answer(text=f"Welcome <a href='tg://user?id={message.from_user.id}'> admin </a>,"
                                 f" please choose command from commands list")
        else:
            await message.bot.set_my_commands(commands=commands_user)
            await message.answer(text=f"Welcome <a href='tg://user?id={message.from_user.id}'> "
                                      f"{user[3]}</a>")
    else:
        await message.answer(text="Iltimos ro'yxatdan o'ting!", reply_markup=kb_register)
        await state.set_state(RegisterStates.startReg)
